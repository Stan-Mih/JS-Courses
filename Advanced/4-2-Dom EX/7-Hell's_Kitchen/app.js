function solve() {
   ...
   function onClick () {     
      ...
      input.forEach((el) => {                   //creatng objects
      ...
      function avrgPay(obj) {                                      // avrg pay FUNCTION
         let sum = 0;
      ...
      function bestSalary (obj) {                              // best pay FUNCTION
         const arr = [];
         for (const key in obj) {
      ...
      document.getElementById("bestRestaurant").textContent = `Name: ${bestName} Average Salary: ${avrgSalary} Best Salary: ${bestSalary(sortedRest[0])}`;

      const rest = sortedRest[0];
      const keys = Object.keys(rest);
      const index = keys.indexOf("restaurant");
      keys.splice(index,1);
